...
def qc_summary(
    ...
    from vocab_qc.core.models import QcRuleResult
    ...
    try:
        ...
        for row in query.all():
            passed_count = (
                ...
                .filter(
                    QcRuleResult.rule_id == row.rule_id,
                    QcRuleResult.dimension == row.dimension,
                    QcRuleResult.passed.is_(True),
                )
                .scalar()
            )
            failed = row.total - passed_count
            rate = f"{passed_count / row.total * 100:.1f}%" if row.total > 0 else "N/A"
            table.add_row(row.rule_id, row.dimension, str(row.total), str(passed_count), str(failed), rate)
