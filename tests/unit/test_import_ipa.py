...
class TestImportWithIPA:
    def test_import_with_ipa(self, db_session):
        ...
        assert apple_phonetic.ipa_uk == "/ˈæp.əl/"

        # 验证 beautiful 的 IPA（英美双音标）
        ...
        assert beautiful_phonetic.ipa_uk == "/ˈbjuː.tə.fəl/"
        assert beautiful_phonetic.ipa_us == "/ˈbjuː.tə.fəl/"
    ...
    def test_csv_import_with_ipa(self, db_session):
        """验证 CSV 导入支持 ipa 列（旧格式兼容映射到 ipa_uk）。"""
        ...
        assert orange_phonetic.ipa_uk == "/ˈɔr.ɪndʒ/"

    def test_excel_import_with_ipa(self, db_session):
        """验证 Excel 导入支持 ipa 列（旧格式兼容）。"""
        import io
        ...
        assert book_phonetic.ipa_uk == "/bʊk/"
